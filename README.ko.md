...
- **토큰 통계** — 세션별 입력 / 출력 / 캐시 토큰 수
- **전체 텍스트 검색** — `⌘K`로 변경된 세션만 다시 만드는 로컬 인덱스 검색
...
- **이미지 표시** — 포함된 이미지는 로컬로 표시；외부 URL 이미지는 클릭해 허용한 후에만 로드
...
- **북마크 URL** — 같은 브라우저와 컴퓨터에서 정확한 세션과 스크롤 위치를 다시 열 수 있습니다 (`?p=&s=&scroll=`). URL에는 세션 데이터나 로컬 파일 접근 권한이 포함되지 않습니다.
